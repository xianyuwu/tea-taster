...
| ✏️ 茶样编辑 | 弹窗编辑茶样信息，字段可在后台自由配置（增删、排序） |
| 💰 单价对比 | 根据价格和净重自动计算元/克，色块渐变直观对比性价比。派生指标可在后台自定义 |
...
| ⚙️ 后台管理 | 大模型配置、评分维度自定义、茶样字段配置、派生指标配置、系统提示词编辑、数据备份恢复 |
...
**管理后台**（`/admin`）— 大模型配置、提示词编辑、数据管理、评分维度配置、茶样字段配置、派生指标配置
...
```
tea-taster/
├── server.py           # Flask 后端服务（API + 静态文件，~780 行）
├── index.html          # 品鉴页面（打分、对比、排名、AI 分析，~3700 行）
├── admin.html          # 管理后台（配置、备份、维度/字段/指标管理，~900 行）
...
| `GET` | `/api/tea-fields` | 获取茶样字段配置 |
| `PUT` | `/api/tea-fields` | 更新茶样字段配置 |
| `GET` | `/api/derived-metrics` | 获取派生指标配置 |
| `PUT` | `/api/derived-metrics` | 更新派生指标配置 |
...
进入管理后台 →「茶样字段配置」，可自由增删字段、设置类型（文本/数字）、显示单位和对齐方式。已有茶样数据不受影响。

**Q: 如何自定义派生指标（如单价）？**

进入管理后台 →「派生指标配置」，可配置由茶样字段自动计算的指标。每个指标指定分子字段和分母字段，当引用的字段被删除时指标自动隐藏。
